...
.addField("__**NOT 2 **__", `Eğer bütün herşey ayarlandıysa kullanıcı \`${p}sipariş yap\` yazdığında sorular sorulmaya başlanacaktır.`))
  ...
  let kim3rtd = cevap1.first().content;
    if (kim3rtd === "iptal") return message.channel.send("✅ | **İşlem iptal edildi!**").then(sa => abc.delete(50))
    ...
    let kid2 = cevap2.first().content;
      if (kid2 === "iptal") return message.channel.send("✅ | **İşlem iptal edildi!**").then(() => abc.delete(50))
...
if (kid3 === "iptal") return message.channel.send("✅ | **İşlem iptal edildi!**").then(() => abc.delete(50))
    ...
    let km3rtid4 = cevap4.first().content;
      if (km3rtid4 === "iptal") return message.channel.send("✅ | **İşlem iptal edildi!**").then(() => abc.delete(50))
    ...
    .setColor("#ffff18")
      await abc.edit(embed4)
           client.channels.get(kanall).sendEmbed(embed4)
           abc.delete();
          ...
          message.author.sendEmbed(embed4)
.catch(err => {message.channel.send("📛 | **Mesaja verilen süre içinde cevap verilmedi!**").then(r => r.delete(10000)
 )})})})})})}}
exports.conf = {
  enabled: true, 
  guildOnly: true, 
