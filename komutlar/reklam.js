...
if (!message.guild) {
  ...
  if (!args[0]) return message.channel.send(`:white_check_mark: Reklam Filtresini Ayarlamak İçin \`+reklam aç\` | Kapatmak İstiyorsanız \`+reklam kapat\` Yazabilirsiniz.`)
  if (args[0] !== 'aç' && args[0] !== 'kapat') return message.channel.send(`:white_check_mark Reklam Filtresini Ayarlamak İçin \`+reklam aç\` | Kapatmak İstiyorsanız \`+reklam kapat\` Yazabilirsiniz.`)
