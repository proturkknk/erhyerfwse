...
if(message.channel.id !== "kullanılacak kanal idsi")return message.channel.send(new Discord.MessageEmbed() .setDescription(`<#kanal idsi> Burada yapabilirsin :x:`).setAuthor(message.author.tag ,message.author.avatarURL).setColor("PURPLE")).then(m => m.delete(5000));
            ...
            msg.edit(':eight_pointed_black_star:| **Şimdi de ödülü yaz bakalım**').then(msg => {
              message.channel.awaitMessages(filter, {
                ...
                try {
                  ...
                  .setFooter(message.author.username + " (Xaine çekiliş sistemi)", message.author.avatarURL);
                  message.guild.channels.find("name" , room).send(' :heavy_check_mark: **ÇEKİLİŞ BAŞLADI** :heavy_check_mark:' , {embed: giveEmbed}).then(m => {
                     ...
                     setTimeout(() => {
                       ...
                       .setTitle(title)
                       .setColor("#f558c9")
            .setFooter("(Xaine çekiliş sistemi)")
...
.setTimestamp()
                     m.edit('** 🎉 ÇEKİLİŞ BİTTİ 🎉**' , {embed: endEmbed});
                       ...
                       var embedLel = new Discord.RichEmbed()
                        .setColor("#f558c9")
                        .setDescription("Ödülünü Moderatörleri Etiketleyerek Alabilirsin!").setFooter("(Xaine çekiliş sistemi)")
